...
public final class MobileShellComposite: MobileTerminalOutputSinking {
    ...
    public var supportsTerminalPaste: Bool { supportedHostCapabilities.contains(Self.terminalPasteCapability) }
    #if DEBUG
    /// Whether the connected Mac advertises the `dogfood.checklist` capability
    /// (the P2 checklist push/fetch verbs). Computed from the same
    /// `supportedHostCapabilities` set as the other capability flags so the DEV
    /// checklist subscription never needs its own status round-trip.
    // Internal (not private): used by the dogfood-feedback extension file.
    var supportsDogfoodChecklist: Bool { supportedHostCapabilities.contains(Self.dogfoodChecklistCapability) }
    #endif
    ...
    // Internal (not private): used by the dogfood-feedback extension file.
    weak var dogfoodFeedbackModel: DogfoodFeedbackModel?
    ...
    // Internal (not private): used by the dogfood-feedback extension file.
    var dogfoodChecklistListenerTask: Task<Void, Never>?
    #endif
    ...
    /// Whether the given surface is scrolled up (has room to jump to the live
    /// bottom), driving the floating jump-to-bottom button's visibility.
    ///
    /// Defaults to `false` (button hidden) when no frame has reported a position
    /// yet, so the affordance never appears spuriously on a fresh attach.
    ...
    /// the jump-to-latest tap routes to the Mac, which scrolls its surface and
    ...
    public func scrollTerminalToBottom(surfaceID: String) async {
        ...
        do {
            ...
            mobileShellLog.error("scroll-to-bottom forward failed surface=\(surfaceID, privacy: .public) error=\(String(describing: error), privacy: .public)")
        }
    }
    ...
    public func connectPairingURLResult(_ rawValue: String? = nil) async -> MobilePairingURLConnectionResult {
        ...
        do {
            ...
            if case MobileSyncPairingPayloadError.loopbackRouteRejected = error {
                ...
                applyPairingFailure(.unrecognizedVersion, phase: "validation")
            } else {
    ...
    private func resetTerminalOutputTracking() {
        ...
        // Clearing the whole capability set zeroes every computed support flag
        // (paste/groups/actions/read-state/close/dogfood) so a reconnect to an
        // older host cannot act on a stale `true` before the next status probe.
    ...
    private func handleTerminalRenderGridEvent(_ event: MobileEventEnvelope) {
        ...
        // Track the authoritative viewport position the Mac stamped on this
